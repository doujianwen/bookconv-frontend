const fs = require('fs');
const report = # 外链自动提交可行性最终报告
## 日期: 2026-08-05

## 一、核心结论

经过对12个外链目录站点的深度检查，只有 TechAsoft 具备自动提交条件，但实际提交仍返回404（JS渲染表单）。
...
| 站点 | URL | 状态 | 表单类型 | 说明 |
|------|-----|------|----------|------|
| TechAsoft | https://www.techasoft.com/submit | 200 | 联系表单 | 有lead_url字段，但GET提交返回404，需JS处理 |

### 2. 需手动提交 (8个)

| 站点 | URL | 原因 |
|------|-----|------|
| PRLog | https://www.prlog.org/ | JS渲染表单，无公开API，所有/api/*端点返回301 |
...
| SaaSPedia | https://saaspedia.io/ | 仅有联系表单，无提交页面 |
| BacklinkCRM | https://backlinkcrm.io/ | 仅有联系表单，无提交页面 |
...
3. 其他站点: 全部手动提交
4. StartupStash: 需配置完整浏览器会话（cookies/headers）才能访问
