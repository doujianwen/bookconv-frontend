...
EPUB is supported by Apple Books, Kobo, Nook, Google Play Books, PocketBook, and Kindle through Send to Kindle. It is the closest thing publishing has to a universal format.`
    },
    {
      ...
      body: `**1. Upload the RTF.** Drag it in or click to browse. Free accounts cover files up to roughly 50MB, and since RTF stores images inefficiently as hex, that limit matters more here than with most formats — an illustrated RTF can balloon fast.
...
One thing worth checking before you upload: if your RTF fakes headings by making text big and bold instead of applying a real heading style, the converter has nothing to build a table of contents from. Five minutes fixing styles produces a noticeably better ebook.`
  ...
  faq: [
    ...
    { q: 'Is there a file size limit?', a: 'Free accounts handle files up to about 50MB, which is plenty for text-heavy documents. Image-heavy RTFs can exceed that because of the hex encoding — Pro accounts raise the limit and add batch conversion.' }
  ]
};
